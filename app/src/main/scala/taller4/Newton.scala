...
object Newton{
  ...
  def raizNewton(e: Expr, a: Atomo, x0: Double, tol: Double = 1e-7, maxIter: Int = 1000): Double = {

    def isCloseEnough(value: Double): Boolean = Math.abs(value) < tol
    def newtonIter(x: Double, iter: Int): Double = {
      if (iter >= maxIter)
        throw new ArithmeticException("Se alcanzó el máximo de iteraciones. No se encontró solución.")
      else {
        val fValue = evaluar(e, a, x)
        if (isCloseEnough(fValue)) x
        else {
          val fPrime = evaluar(derivar(e, a), a, x)
          newtonIter(x - fValue / fPrime, iter + 1)
        }
      }
    }
